...
const config = (request, reply) => {
    ...
    if (payload.watson_username) {
        process.env.WATSON_USERNAME = SpeechToText.WatsonSTT._options.username = payload.watson_username;
    }

    if (payload.watson_password) {
        process.env.WATSON_USERNAME = SpeechToText.WatsonSTT._options.password = payload.watson_password;
    }
